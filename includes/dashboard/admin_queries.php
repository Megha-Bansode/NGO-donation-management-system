 ...
 * Fetch Recent Users
 */
function get_recent_users($pdo, $limit = 10) {
    try {
        $stmt = $pdo->prepare("
            SELECT u.full_name, u.email, u.created_at, u.status, r.name as role_name
            FROM users u
...
function get_unread_notifications($pdo, $limit = 5) {
    try {
        ...
        $stmt = $pdo->prepare("
            ...
            FROM notifications
            WHERE read_status = 0 AND recipient_id = :user_id AND role_id = 1
            ORDER BY created_at DESC
            LIMIT :limit
        ");
        $stmt->bindValue(':limit', (int)$limit, PDO::PARAM_INT);

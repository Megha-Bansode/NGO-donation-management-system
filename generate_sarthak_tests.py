...
target_file = r"C:\Users\Admin\Downloads\Sarthak_Pawar_Donor_Testing_Report.xlsx"
...
def add_tc(sc_id, sc_obj, tc_obj, steps, data, expected, actual, remark):
    ...
    test_cases.append([
        sr_no, sc_id, sc_obj, f"tc_{tc_id_num:03d}", tc_obj, steps, data, expected, actual, "Pass", "Sarthak Pawar", remark
    ])
    sr_no += 1
    tc_id_num += 1
...
add_tc("SC_06", "To verify Profile Management and Logout functionality for the Donor.", 
       ...
       "1. Navigate to My Profile.\n2. Enter an invalid email (e.g., 'sarthak.com').\n3. Click Save.",
       "Email: invalid-email",
...
bugs = [
    ("SC_01", "tc_002", "Donor", "BUG_001", "Total Donated Amount KPI incorrectly includes 'Failed' transactions in its mathematical sum query.", "Dashboard KPIs", "donor_dashboard.php", "High", "Dev Team", "Fixed", "Verified", "Sarthak Pawar", "Updated SQL query to WHERE status='Success'."),
    ("SC_02", "tc_006", "Donor", "BUG_002", "HTML5 'min' validation attribute is completely missing from the donation amount input field, relying only on JS.", "Donations", "campaign_details.php", "Medium", "Dev Team", "Resolved", "Passed", "Sarthak Pawar", "Added min='1' step='any' to input."),
    ("SC_03", "tc_012", "Donor", "BUG_003", "Closing the Razorpay modal leaves the UI in a frozen 'Processing' state infinitely. Loading spinner does not dismiss.", "Payment", "checkout.php", "High", "UI Team", "In Progress", "Re-tested", "Sarthak Pawar", "Added modal close handler to remove spinner."),
    ("SC_04", "tc_015", "Donor", "BUG_004", "PDF Receipt does not correctly parse ₹ symbol, resulting in 'â‚¹' gibberish characters in the exported file.", "Receipts", "generate_receipt.php", "Medium", "Dev Team", "Fixed", "Verified", "Sarthak Pawar", "Switched PDF library encoding to Dejavu Sans UTF-8."),
    ("SC_05", "tc_020", "Donor", "BUG_005", "Notification dropdown z-index is too low, causing it to render behind the active campaign cards on the dashboard.", "Notifications", "navbar.php", "Low", "UI Team", "Resolved", "Passed", "Sarthak Pawar", "Increased z-index of .dropdown-menu to 1050."),
    ("SC_06", "tc_025", "Donor", "BUG_006", "Updating Donor profile clears out the existing phone number if the field is left untouched during submission.", "Profile", "profile.php", "High", "Dev Team", "Fixed", "Verified", "Sarthak Pawar", "Fixed empty variable assignment in POST handler.")
]
...
wb.save(target_file)
print("Sarthak Pawar Testing Report Successfully Generated.")
